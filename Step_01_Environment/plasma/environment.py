...
class PlasmaEnvironment:
    """
    HL-2A-informed reduced-order plasma-like environment.

    Main pattern:
        N gradually decreases
        D gradually increases
        mode_amp grows as hidden dominant MHD mode amplitude
        collapse risk rises
    """
    ...
    def step(self, u: float):
        ...
        u > 0 means stabilizing action:
            supports N,
            suppresses D,
            suppresses hidden mode_amp,
            consumes E.

        u = 0 means no active correction.
        """
        ...
        # Natural HL-2A-informed drift
        natural_N_decay = 0.035
        natural_D_growth = 0.030

        # Hidden dominant MHD mode grows slowly when internal tension accumulates.
        ...
        # Mode grows faster when D starts dominating N.
